...
class CenterCrop(object):
    ...
    def __call__(self, inputs, target):
        ...
        inputs[2] = inputs[1][y1: y1 + th, x1: x1 + tw]
...
class Scale(object):
    ...
    def __call__(self, inputs, target):
        ...
        inputs[2] = ndimage.interpolation.zoom(inputs[2], ratio, order=self.order)
...
class RandomRotate(object):
    ...
    def __call__(self, inputs, target):
        ...
        angle = applied_angle - diff/2

        h, w, _ = target.shape

        inputs[0] = ndimage.interpolation.rotate(inputs[0], angle, reshape=self.reshape, order=self.order)
        inputs[1] = ndimage.interpolation.rotate(inputs[1], angle, reshape=self.reshape, order=self.order)
        inputs[2] = ndimage.interpolation.rotate(inputs[2], angle, reshape=self.reshape, order=self.order)
        target = ndimage.interpolation.rotate(target, angle, reshape=self.reshape, order=self.order)
        return inputs, target
...
class RandomColorWarp(object):
    ...
    def __call__(self, inputs, target):
        ...
        inputs[2] *= (1 + random_std)
        inputs[2] += random_mean

        inputs[0] = inputs[0][:, :, random_order]
        inputs[1] = inputs[1][:, :, random_order]
        inputs[2] = inputs[2][:, :, random_order]
